...
### example 3
[example 1](#example-1)과 [example 2](#example-2)에서는 `--dataset` 또는 `--trainset`, `--testset`을 입력받지 않았을 때 프로그램이 자체적으로 사용자에게 데이터에 대한 정보를 요청할 때의 예시입니다.<br>
...
### data input example 
[example 1](#example-1)과 [example 2](#example-2)에서 데이터 경로 입력의 예<br>
