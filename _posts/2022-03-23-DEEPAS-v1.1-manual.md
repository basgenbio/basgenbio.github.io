...
### example3
example1과 example2에서는 `--dataset` 또는 `--trainset`, `--testset`을 입력받지 않았을 때 프로그램이 자체적으로 사용자에게 데이터에 대한 정보를 요청할 때의 예시입니다.<br>
example3는 `--trainset`과 `--testset` 옵션을 이용한, 하이퍼파라미터 튜닝을 하지 않을 때의 예시입니다.
```shell
(deepas-1.1)$ python program.py \
--model dnn \
--index_column ID \
...
### data input example 
example1과 example2에서 데이터 경로 입력의 예<br>
