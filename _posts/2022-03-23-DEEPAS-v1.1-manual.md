...
- [**updated**][execution in Terminal](#execution-in-Terminal)
...
- [User Guide](#user-guide)
  ...
  - [`FeatureImportance`](#FeatureImportance)
    - [`explain`](#explain)
- [**updated**][Terminal Argument Option](#Terminal-Argument-Option)
  - [`argument`](#argument)
...
- `--device`: type=str, default='cpu'
  - 사용할 자원의 종류를 입력합니다. `gpu` 또는 `cpu`만 입력할 수 있습니다.<br><br>
...
- `--tuning_method`: type=str, default=**False**)
  - 하이퍼파라미터 튜닝을 하고자 할 경우 해당 옵션에 사용할 기법을 입력합니다. 입력 가능한 값은 `grid`, `bayesian` 입니다. 해당 옵션을 입력하지 않을 경우 또는 `--force_stop_tuning` 옵션을 `True`로 설정할 경우 하이퍼파라미터 튜닝은는 수행되지 않습니다.<br><br>
...
- `--verbose`: type=int, default=2
  - `--model` 옵션이 `dnn`일 경우에 유의한 옵션입니다. dnn의 학습과정에 대한 터미널 창의 메세지에 대한 빈도수를 조절할 수 있습니다. 낮은 값을 줄수록 더 간결한 화면을 유지할 수 있습니다.<br><br>
