...
- [execution in Terminal](#execution_in_Terminal)
  - [how to run](#how_to_run)
    - [exmaple 1](#exmaple_1)
    - [exmaple 2](#exmaple_2)
    - [data input example](#data_input_example)
- [User Guide](#user-guide)
  ...
  - [`FeatureImportance`](#FeatureImportance)
    - [`explain`](#explain)
  - [Terminal Argument Option](#Terminal_Argument_Option)
...
- 입력해야 하는 데이터의 경로는 deepas v1.0의 예제 코드 상에서 KCPS 데이터 사용 시의 merged_train_df, merged_val_df 변수와 같은 데이터입니다. genotype과 phenotype 데이터가 한 데이터 내에 있어야 하며, 전처리가 완료된 데이터를 input으로 넣어야 합니다.
- hyperparameter 옵션 입력 시, 반드시 큰 따옴표로 감싼 후 dict 형식으로 입력해야 하며, 모델 별 하이퍼파라미터를 작은 따옴표 처리해야 합니다.
## how to run
...
데이터 경로 입력의 예)
```
please input dataset path or input done: your_train_data_path
please input type [train or test or all]: train
please input seperator [, or \t]: \t

please input dataset path or input done: your_test_data_path
please input type [train or test or all]: train
please input seperator [, or \t]: \t
```
